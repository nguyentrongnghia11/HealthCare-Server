...
export class PostsService {
  ...
  async findAll(): Promise<PostItem[]> {
    ...
    return docs.map((d) => ({ id: d.id, title: d.title, excerpt: d.excerpt, content: d.content, image: d.image, date: d.date }));
  }

  async findOne(id: number): Promise<PostItem> {
    const post = await this.postModel.findOne({ id }).lean().exec();
    if (!post) throw new NotFoundException('Post not found');
    return { id: post.id, title: post.title, excerpt: post.excerpt, content: post.content, image: post.image, date: post.date };
  }
  ...
  async create(payload: Omit<PostItem, 'id'>): Promise<PostItem> {
    ...
    return { id: created.id, title: created.title, excerpt: created.excerpt, content: created.content, image: created.image, date: created.date };
  }
  ...
  async update(id: number, payload: Partial<PostItem>): Promise<PostItem> {
    ...
    return { id: updated.id, title: updated.title, excerpt: updated.excerpt, content: updated.content, image: updated.image, date: updated.date };
  }
